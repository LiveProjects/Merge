...
if (isset ( $com_name ) && isset ( $start ) && isset ( $end )) {
	...
	// echo $res_com['FID'];DIE;
	// t_hs_overwork_reserv与t_hs_employee联合查询，找出：提报人姓名、人数、日期、时间、上车站点
	$sql = "select a.FName,b.FNum,b.FRDate,b.FRTime,b.FStopID as FStop from
	 t_hs_college_reserv as b inner join t_hs_employee as a on b.FNumber=a.FNumber
...
if ($type == 'excel') {
	...
	$objSheet->setTitle ( "海信学院班车统计" ); // 设置sheet名称
	                                 
	// 设计表格标题
	$objSheet->setCellValue ( "A1", "海信学院班车统计" ); // 表格标题
	...
	// 设置表头
	$objSheet->setCellValue ( "A2", "公司" )->setCellValue ( "B2", "提报人姓名" )->setCellValue ( "C2", "人数" )->setCellValue ( "D2", "日期" )->setCellValue ( "E2", "时间" )->setCellValue ( "F2", "上车站点" );
	...
	foreach ( $data as $excel_k => $excel_v ) {
		$index = 0;
		$objSheet->setCellValue ( getCells ( $index ) . $j, $excel_v ['com_name'] )->setCellValue ( getCells ( ++ $index ) . $j, $excel_v ['emp_name'] )->setCellValue ( getCells ( ++ $index ) . $j, $excel_v ['FNum'] )->setCellValue ( getCells ( ++ $index ) . $j, $excel_v ['FRDate'] )->setCellValue ( getCells ( ++ $index ) . $j, $excel_v ['FRTime'] )->setCellValue ( getCells ( ++ $index ) . $j, $excel_v ['FStop'] );
		++ $j;
	}
