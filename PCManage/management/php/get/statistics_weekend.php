...
if ($type == 'excel') {
	...
	$objSheet->getStyle ( 'A1:E1' )->getFill ()->setFillType ( PHPExcel_Style_Fill::FILL_SOLID )->getStartColor ()->setRGB ( '098079' ); // 设置单元格背景颜色
	...
	$objSheet->getStyle ( 'A2:E2' )->getFill ()->setFillType ( PHPExcel_Style_Fill::FILL_SOLID )->getStartColor ()->setRGB ( '9fb8b7' ); // 设置表头单元格背景颜色
	                                     // 设置表头单元格边框
	...
	foreach ( $excel as $excel_k => $excel_v ) {
		$index = 0;
		$objSheet->setCellValue ( getCells ( $index ) . $j, $excel_v ['com_name'] )->setCellValue ( getCells ( ++ $index ) . $j, $excel_v ['FSection'] )->setCellValue ( getCells ( ++ $index ) . $j, $excel_v ['emp_name'] )->setCellValue ( getCells ( ++ $index ) . $j, $excel_v ['FRDate'] )->setCellValue ( getCells ( ++ $index ) . $j, $excel_v ['FStop'] );
		++ $j;
	}
