...
if (isset ( $com_name ) && isset ( $start ) && isset ( $end )) {
	...
	$sql = "select d.FRDate,d.FStopID as FStop,c.FName as emp_name,c.FSection from
	...
	where( d.FRDate<='{$end}' and d.FRDate>='{$start}') and d.FType='2' order by FRDate asc";
	$res = $db->execsql ( $sql );
	// var_dump($res);
	$web=array();//用于网页显示的数组
...
if ($type=='web'){
	...
	$data=array("web"=>$web,"res_num"=>$res_num);
// 	var_dump($data);
...
if ($type=='excel'){
	...
	->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);//设置字体默认居中
	$objSheet->setTitle("周末加班");//设置sheet名称

	//设计表格标题
	$objSheet->setCellValue("A1","周末加班车统计");//表格标题
	$objSheet->mergeCells('A1:E1');//合并单元格
	$objSheet->getStyle('A1:E1')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)
	->getStartColor()->setRGB('098079');//设置单元格背景颜色
	$objSheet->getStyle('A1:E1')->getFont()->setSize(20)->setBold(TRUE);//设置标题字体样式：大小20，加粗
	$objSheet->getStyle('A1:E1')->applyFromArray(getBorderStyle('1c2a29'));//设置标题单元格边框

	//设置表头
	$objSheet->setCellValue("A2","公司")->setCellValue("B2","部门")->setCellValue("C2","人名")->setCellValue("D2","日期")->setCellValue("E2","上车站点");
	$objSheet->getStyle('A2:E2')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)
	->getStartColor()->setRGB('9fb8b7');//设置表头单元格背景颜色
	//设置表头单元格边框
	...
	$objSheet->getStyle('A2:E2')->getFont()->setSize(16)->setBold(TRUE);//设置表头字体样式：大小20，加粗

	//向表格内填充数据
	$j=3;
	foreach ($excel as $excel_k=>$excel_v){
		$index=0;
		$objSheet->setCellValue(getCells($index).$j,$excel_v['com_name'])
		->setCellValue(getCells(++$index).$j,$excel_v['FSection'])
		->setCellValue(getCells(++$index).$j,$excel_v['emp_name'])
		->setCellValue(getCells(++$index).$j,$excel_v['FRDate'])
		->setCellValue(getCells(++$index).$j,$excel_v['FStop']);
		++$j;
	}
