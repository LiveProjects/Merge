...
if (isset ( $com_name ) && isset ( $start ) && isset ( $end )) {
	...
	$sql = "select d.FNumber as emp,d.FNum,d.FRDate,d.FRTime,d.FStartStop,d.FEndStop,d.FType from
	 (select b.FNumber from t_hs_company as a inner join t_hs_employee as b on a.FID=b.FCompanyID where a.FName='hisense2')as c 
	INNER  JOIN t_hs_temporary_reserv as d on c.FNumber=d.FNumber 
	where( d.FRDate<='{$end}' and d.FRDate>='{$start}')";
	$res = $db->execsql ( $sql );
	foreach ( $res as $key => $value ) {
		$sql_emp = "select FName from t_hs_employee where FID='{$value['emp']}'";
		$res_emp = $db->getrow ( $sql_emp );
		$res [$key] ['emp'] = $res_emp ['FName'];
	}
// 	var_dump($res);
